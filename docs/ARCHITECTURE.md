...
- **On disconnect** it keeps current state fresh for one polling window (a drop alone is not evidence of change), refreshes immediately and polls at the configured interval. On the owner's account an intermediary closes the stream after about 120 s without an update, so quiet periods reconnect roughly every two minutes; once a status time is known, the refreshes around a drop and reconnect read the cloud cache (observed: 120.0 s connections, no escalations). Reconnects back off from 1 second to 5 minutes and honor retry-after; a connection that lasted a minute resets the backoff. A stream silent for five minutes (no data or comments) is treated as dead.
